...
ZONE_ID=$(echo "$ZONE_INFO" | jq -r --arg name "$DOMAIN" '.result[] | select(.name == $name) | .id')
...
echo "⚠️ Lưu ý: Nếu Docker vẫn không hoạt động do quyền, hãy chạy: newgrp docker hoặc sudo reboot"
